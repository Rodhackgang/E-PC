  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Filtrer les catégories pour celles nommées "QCM PC"
        const qcmPcCategories = categories.filter(category => category.title === 'QCM PC');

        const loadedQuestions = qcmPcCategories.flatMap(cat =>
          cat.questions.map(q => ({
            question: q.text,
            options: q.options
          }))
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>QCM: QCM PC</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  actionButton: {
    backgroundColor: '#009688',
    padding: 12, // Adjust padding to reduce button size
    borderRadius: 8,
    alignItems: 'center',
    flex: 1,
    marginHorizontal: 8,
  ...
  actionButtonText: {
    color: '#fff',
    fontSize: 16, // Adjust font size to match button size
    fontWeight: '700',
  },
  noQuizContainer: {
    flex: 1,
...
export default DetailsListqcmpc;
